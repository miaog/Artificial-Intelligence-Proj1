...
class CornersProblem(search.SearchProblem):
    ...
    def goalTest(self, state):
        ...
        """
        if len(state[1]) == 4 and state[0] in self.corners:
            return True
        else:
            return False
    ...
    def getResult(self, state, action):
        ...
        if hitsWall == False:
            ...
            visited = state[1]
            if new_position in self.corners and new_position not in visited:
                visited = visited + ((nextx, nexty),)
            return (new_position, visited)
        else:
            return state[0]
...
def cornersHeuristic(state, problem):
    ...
    while unvisited:
        for corner in unvisited:
            manhattan_distance = util.manhattanDistance(closest_state, corner)
            manhattan_distances.append((manhattan_distance, corner))
        next_corner = min(manhattan_distances)
        lowest_cost, current_corner = next_corner
        manhattan_distances = []
        unvisited.remove(current_corner)
        total_distance += lowest_cost
        closest_state = current_corner
